...
@section('title', 'Approval Ketua - Dispensasi')
...
<div class="space-y-6">
    ...
    <div class="space-y-4">
        <div class="flex items-center justify-between">
            <div>
                <h1 class="text-3xl font-bold text-gray-900">Approval Dispensasi</h1>
                <p class="text-gray-600 mt-1">Approval final dispensasi dari semua bidang</p>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div class="bg-purple-50 border border-purple-200 rounded-lg p-4">
                <div class="flex items-center justify-between">
                    <div>
                        <p class="text-xs font-semibold text-purple-600 uppercase tracking-wider">Total Peserta</p>
                        <p class="text-2xl font-bold text-purple-900 mt-1">
                            {{ $dispensasis->sum(function($d) { return count($d->user_ids ?? []); }) }}
                        </p>
                    </div>
                ...
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-2xl font-bold text-green-900 mt-1">
                            {{ $dispensasis->pluck('bidang_id')->unique()->count() }}
                        </p>
                    </div>
            ...
            <table class="w-full" id="dispensasiTable">
                <thead class="bg-gray-50 border-b border-gray-200">
                    <tr>
                        ...
                        <th class="px-6 py-4 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">Direview Sekretaris</th>
                ...
                <tbody class="divide-y divide-gray-200" id="dispensasiTableBody">
                    @forelse($dispensasis as $index => $dispensasi)
                        <tr class="hover:bg-gray-50 transition">
                            <td class="px-6 py-4 text-sm text-gray-900">{{ $dispensasis->firstItem() + $index }}</td>
                            ...
                            <td class="px-6 py-4">
                                ...
                                @if($dispensasi->submittedBy)
                                    <div class="text-xs text-gray-500 mt-1">
                                        oleh {{ $dispensasi->submittedBy->name }}
                                    </div>
                                @endif
                            </td>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="text-xs text-gray-600">
                                    {{ $dispensasi->reviewedBySekretaris->name ?? '-' }}
                                </div>
                                <div class="text-xs text-gray-500">
                                    {{ $dispensasi->reviewed_at_sekretaris ? $dispensasi->reviewed_at_sekretaris->format('d M Y H:i') : '-' }}
                                </div>
                            </td>
                            
                            <td class="px-6 py-4">
                                <div class="flex items-center justify-center space-x-2">
                                    <button onclick="openDetailModalKetua({{ $dispensasi->id }})"
                                    ...
                                    <button onclick="openApproveModal({{ $dispensasi->id }}, '{{ $dispensasi->pengajuanBudget->nama_aksi }}')"
                                    ...
                                    <button onclick="openRejectModal({{ $dispensasi->id }}, '{{ $dispensasi->pengajuanBudget->nama_aksi }}')"
                    ...
                    @empty
                        <tr>
                            <td colspan="7" class="px-6 py-16 text-center">
                                ...
                                <p class="mt-4 text-gray-600 font-semibold">Tidak ada dispensasi yang menunggu approval</p>
                                <p class="text-gray-500 text-sm">Semua dispensasi sudah di-approve</p>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
...
@include('ketua.dispensasi.approve-modal')
@include('ketua.dispensasi.reject-modal')
...
<script>
    ...
    let currentDispensasiId = null;
    let currentDispensasiName = '';

    function openApproveModal(id, name) {
        currentDispensasiId = id;
        currentDispensasiName = name;
        
        document.getElementById('approveDispensasiName').textContent = name;
    ...
    function closeApproveModal() {
        ...
        setTimeout(() => {
            ...
            currentDispensasiId = null;
            currentDispensasiName = '';
        }, 250);
    }
    ...
    function openRejectModal(id, name) {
        ...
        document.getElementById('rejectDispensasiName').textContent = name;
    ...
    async function openDetailModalKetua(id) {
        // Pakai fungsi openDetailModal yang sama dari detail modal
    ...
    document.getElementById('approveForm').addEventListener('submit', async function(e) {
        ...
        try {
            const response = await fetch(`/ketua/dispensasi/${currentDispensasiId}/approve`, {
                method: 'POST',
                headers: {
    ...
    document.getElementById('rejectForm').addEventListener('submit', async function(e) {
        ...
        try {
            const response = await fetch(`/ketua/dispensasi/${currentDispensasiId}/reject`, {
                method: 'POST',
                headers: {
                    'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content,
                },
