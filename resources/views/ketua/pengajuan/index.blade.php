...
@section('title', 'Approval Ketua - Pengajuan Budget')
...
<div class="space-y-6">
    ...
    <div class="space-y-4">
        <div class="flex items-center justify-between">
            <div>
                <h1 class="text-3xl font-bold text-gray-900">Approval Pengajuan Budget</h1>
                <p class="text-gray-600 mt-1">Approval final pengajuan budget dari semua bidang</p>
            </div>
        </div>
            ...
            <table class="w-full" id="pengajuanBudgetTable">
                <thead class="bg-gray-50 border-b border-gray-200">
                    <tr>
                        ...
                        <th class="px-6 py-4 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">Acc by Bendahara</th>
                ...
                <tbody class="divide-y divide-gray-200" id="pengajuanBudgetTableBody">
                    @forelse($pengajuanBudgets as $index => $pb)
                        <tr class="hover:bg-gray-50 transition">
                            ...
                            <td class="px-6 py-4">
                                @if($pb->reviewedByBendahara)
                                    <div class="text-xs text-gray-600">
                                        {{ $pb->reviewedByBendahara->name }}
                                    </div>
                                    <div class="text-xs text-gray-500">
                                        {{ $pb->reviewed_at_bendahara->format('d M Y') }}
                                    </div>
                                @else
                                    <span class="text-xs text-gray-400">-</span>
                                @endif
                            ...
                            <td class="px-6 py-4">
                                <div class="flex items-center justify-center space-x-2">
                                    <button onclick="openDetailModalKetua({{ $pb->id }})"
                    ...
                    @empty
                        <tr>
                            <td colspan="8" class="px-6 py-16 text-center">
                                ...
                                <p class="mt-4 text-gray-600 font-semibold">Tidak ada pengajuan budget yang menunggu approval</p>
                                <p class="text-gray-500 text-sm">Semua pengajuan budget sudah di-approve</p>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
...
@include('ketua.pengajuan.approve-modal')
@include('ketua.pengajuan.reject-modal')
@include('ketua.pengajuan.detail')
...
<script>
    ...
    document.getElementById('approveForm').addEventListener('submit', async function(e) {
        ...
        try {
            const response = await fetch(`/ketua/pengajuan/${currentPengajuanId}/approve`, {
                method: 'POST',
                headers: {
    ...
    document.getElementById('rejectForm').addEventListener('submit', async function(e) {
        ...
        try {
            const response = await fetch(`/ketua/pengajuan/${currentPengajuanId}/reject`, {
                method: 'POST',
                headers: {
                    'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content,
                },
