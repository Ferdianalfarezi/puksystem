...
<div id="historyDetailModal" class="fixed inset-0 hidden items-center justify-center z-50 p-4 bg-black bg-opacity-50 backdrop-blur-sm opacity-0 transition-opacity duration-250">
    <div class="bg-white rounded-xl shadow-2xl w-full max-w-5xl max-h-[90vh] overflow-y-auto transform transition-all duration-250">
        ...
        <div class="flex items-center justify-between px-5 py-3 border-b border-gray-200 sticky top-0 bg-white z-10 rounded-t-xl">
            <h2 class="text-lg font-bold text-gray-900">Detail History Program Kerja</h2>
...
let currentHistoryDetailProgramId = null;

async function openHistoryDetailModal(id) {
    currentHistoryDetailProgramId = id;
    ...
    try {
        const response = await fetch(`/history/program-kerja/${id}`, {
            method: 'GET',
            headers: {
...
function renderHistoryDetailContent(pk) {
    ...
    let html = `
        ...
        <div class="grid grid-cols-2 gap-3">
            ...
            <div class="bg-gray-50 rounded-lg p-3">
                ...
                <p class="text-sm font-semibold text-gray-900">${pk.jenis_pengeluaran || '-'}</p>
            </div>
        </div>
    `;
        ...
        <div class="bg-gray-50 border border-gray-200 rounded-lg p-4">
            ...
            <div class="grid grid-cols-2 gap-3 text-xs">
                <div>
                    ...
                    <p class="font-semibold text-gray-900">${pk.created_at_formatted || '-'}</p>
                </div>
                ${pk.submitted_at_formatted ? `
                <div>
                    <p class="text-gray-600 mb-1">Diajukan</p>
                    <p class="font-semibold text-gray-900">${pk.submitted_at_formatted}</p>
                    ${pk.submitted_by_name ? `<p class="text-gray-500 mt-0.5">oleh ${pk.submitted_by_name}</p>` : ''}
                </div>
                ` : ''}
                <div>
                    <p class="text-gray-600 mb-1">Terakhir Update</p>
                    <p class="font-semibold text-gray-900">${pk.updated_at_formatted || '-'}</p>
                </div>
                <div>
                    <p class="text-gray-600 mb-1">Total Aktivitas</p>
                    <p class="font-semibold text-gray-900">${pk.histories_count || 0} aktivitas</p>
                </div>
            </div>
        </div>
    `;
...
function renderHistoryTimeline(pk) {
    if (!pk.histories || pk.histories.length === 0) {
        return `
    ...
    pk.histories.forEach((history, index) => {
        ...
        const isLast = index === pk.histories.length - 1;
...
function closeHistoryDetailModal() {
    ...
    setTimeout(() => {
        ...
        currentHistoryDetailProgramId = null;
    }, 250);
}

// Close modal on ESC key
document.addEventListener('keydown', function(e) {
    if (e.key === 'Escape' && currentHistoryDetailProgramId) {
        closeHistoryDetailModal();
    }
});
</script>