...
<div id="detailModalBendahara" class="fixed inset-0 hidden items-center justify-center z-50 p-4 bg-black bg-opacity-50 backdrop-blur-sm transition-opacity duration-300 opacity-0">
    <div class="bg-white rounded-2xl shadow-2xl w-full max-w-3xl max-h-[90vh] overflow-y-auto transform transition-all duration-300 scale-95">
        ...
        <!-- Modal Body -->
        <div class="p-6 space-y-6" id="detailContentBendahara">
            <!-- Loading State -->
...
async function openDetailModalBendahara(id) {
    ...
    const content = document.getElementById('detailContentBendahara');
    ...
    try {
        console.log('Fetching:', `/bendahara/pengajuan/${id}`); // Debug
        
        const response = await fetch(`/bendahara/pengajuan/${id}`);
