...
@section('title', 'Konfirmasi Sekretaris - Dispensasi')
...
<div class="space-y-6">
    ...
    <div class="space-y-4">
        <div class="flex items-center justify-between">
            <div>
                <h1 class="text-3xl font-bold text-gray-900">Konfirmasi Dispensasi</h1>
                <p class="text-gray-600 mt-1">Review dan konfirmasi dispensasi dari semua bidang</p>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div class="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-2xl font-bold text-yellow-900 mt-1">{{ $dispensasis->total() }}</p>
                    </div>
            ...
            <div class="bg-blue-50 border border-blue-200 rounded-lg p-4">
                <div class="flex items-center justify-between">
                    <div>
                        <p class="text-xs font-semibold text-blue-600 uppercase tracking-wider">Total Peserta</p>
                        ...
                        <tr class="hover:bg-gray-50 transition">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="text-xs text-gray-600">
                                    {{ $dispensasi->submitted_at->format('d M Y') }}
                                </div>
                                <div class="text-xs text-gray-500">
                                    {{ $dispensasi->submitted_at->format('H:i') }} WIB
                                </div>
                            </td>
                            
                            <td class="px-6 py-4">
                                <div class="flex items-center justify-center space-x-2">
                                    <button onclick="openDetailModalSekretaris({{ $dispensasi->id }})"
                    ...
                    @empty
                        <tr>
                            <td colspan="7" class="px-6 py-16 text-center">
                                ...
                                <p class="mt-4 text-gray-600 font-semibold">Tidak ada dispensasi yang menunggu konfirmasi</p>
                                <p class="text-gray-500 text-sm">Semua dispensasi sudah dikonfirmasi</p>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
...
@include('sekretaris.dispensasi.approve-modal')
@include('sekretaris.dispensasi.reject-modal')
...
<script>
    ...
    async function openDetailModalSekretaris(id) {
    ...
    document.getElementById('approveForm').addEventListener('submit', async function(e) {
        ...
        try {
            const response = await fetch(`/sekretaris/dispensasi/${currentDispensasiId}/approve`, {
                method: 'POST',
                headers: {
    ...
    document.getElementById('rejectForm').addEventListener('submit', async function(e) {
        ...
        try {
            const response = await fetch(`/sekretaris/dispensasi/${currentDispensasiId}/reject`, {
                method: 'POST',
                headers: {
                    'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content,
                },
